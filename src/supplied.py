...
BASE=WORK/'supplied/pt002_inflow'
...
def imported_tissue(comm):
    ...
    import ufl
    from .partition import tissue_partitioner
    from .solver import Tissue
    if comm.rank==0:
        with np.load(BASE/'input/prepared.npz') as prepared:
...
def coverage(tissue,graph,radius,circle_degree,path):
    ...
    from .partition import network_partitioner
    comm=tissue.comm
...
def run(args):
    from mpi4py import MPI
    from .solver import Parameters,solve
    comm=MPI.COMM_WORLD
